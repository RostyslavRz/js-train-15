...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    password += characters.at(randomIndex);
...
function calculateCircleArea(radius) {
  ...
  const sqrCircle = Math.PI * Math.pow(radius, 2);
  // const sqrCircle = Math.PI * radius ** 2;  // або так
...
function findMinMax(numbers) {
  if (Array.isArray(numbers) !== true) {
    console.log("Вхідні данні не є массивом");
    return null;
  }
...
function calculateHypotenuse(a, b) {
  if (typeof a !== "number" || typeof b !== "number") {
    console.log("Помилка: a та b повинні бути числами");
    return null;
  }
  ...
  // const hypotenuse = Math.sqrt(a * a + b * b);  // або так
...
function roundObjectValues(obj) {
  ...
  const rounded = Object.fromEntries(map);
...
function calculateVolumeCylinder(radius, height) {
  if (!typeof radius === "number" || !typeof height === "number") {
    console.log("Аргументи повинні бути числом");
    return null;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  const sumInvest = principal * (1 + interestRate) ** years; // Розраховуємо суму інвестицій за формулою P*(1+rate)^years, де P - початкова сума інвестицій, rate - річна процентна ставка, years - кількість років.
  // Округляємо ії до найближчого цілого
  return Math.round(sumInvest); // Повертаємо розраховану суму інвестицій.
}
